...
def kalmanFilter(z_kp1, u, xplus_k, Pplus_k, dt, Q, R):
	...
	H_kp1 = np.array([1,0]) # check these dimensions
	Pminus_kp1 = np.matmul(np.matmul(F_kp1,Pplus_k),np.transpose(F_kp1)) + Q
	nu_kp1 = z_kp1 - xminus_kp1[0]
	S_kp1 = np.matmul(np.matmul(H_kp1,Pminus_kp1),np.transpose(H_kp1)) + R
	K_kp1 = np.matmul(Pminus_kp1,np.transpose(H_kp1))/S_kp1
	#print("F_kp1: ", F_kp1)
	print("Pminus_kp1: ", Pminus_kp1)
	xplus_kp1 = xminus_kp1 + K_kp1*nu_kp1
	Pplus_kp1 = np.matmul((np.identity(2) - np.matmul(K_kp1,H_kp1)),Pminus_kp1)
...
x = np.empty((t.size,4))
xplus = np.empty((t.size,2))
z = np.empty(t.size)
P = np.empty((t.size,2,2))
x[0,:] = np.array([x0, v0, theta0, w0])
xplus[0,:] = np.array([theta0, w0])
P[0,:,:] = np.array( [[1,0], [0,1]])

# Normal Random number generator
sigma_v_theta = 10*2*math.pi/360 # Measurement noise
...
Q = np.array([[sigma_w_theta, 0],[0, sigma_w_omega]])
R = sigma_v_theta
...
plt.plot(t,xplus[:,0], 'g-', linewidth=3)
